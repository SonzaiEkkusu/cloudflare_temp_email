...
const COMMANDS = [
    ...
    {
        command: "new",
        description: "Create a new email address. To customize the email address, enter `/new`. You can specify a domain using `/new <name>@<domain>`. `name` [a-z0-9] is valid; if `name` is empty, a random domain will be generated. `@<domain>` is optional."
    },
    {
        command: "address",
        description: "View a list of email addresses"
    },
    {
        command: "bind",
        description: "To bind an email address, please enter `/bind <email address credentials>`."
    },
    {
        command: "unbind",
        description: "To unbind an email address, please enter `/unbind <email address>`."
    },
    {
        command: "delete",
        description: "To delete an email address, please enter /delete <email address>"
    },
    {
        command: "mails",
        description: "To view emails, please enter /mails <email address>. If no address is entered, the first address will be displayed by default."
    },
    {
        command: "cleaninvalidaddress",
        description: "To clean up invalid addresses, please enter /cleaninvalidaddress"
    },
]
...
export function newTelegramBot(c: Context<HonoCustomType>, token: string): Telegraf {
    ...
    bot.command("start", async (ctx: TgContext) => {
        ...
        return await ctx.reply(
            "Welcome to this robot. You can open the mini app.\n\n"
            + (prefix ? `Prefix is ​​now enabled.: ${prefix}\n` : '')
            + `Currently available domain names: ${JSON.stringify(domains)}\n`
